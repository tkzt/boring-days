<template>
  ...
  >
    <v-col cols="12" md="7" lg="5" xl="4" class="px-0">
        ...
        >
          ...
          <v-menu v-model="avatarMenu">
            <template #activator="{ props }">
              <div style="cursor: pointer" v-bind="props" class="flex-shrink-0">
                <v-avatar ref="avatarRef" size="36" color="primary" />
              </div>
            </template>
            <v-list density="compact">
              <v-list-item
                class="text-caption"
                @click="
                  aboutDialog.model = true;
                  avatarMenu = false;
                "
              >
                关于
              </v-list-item>
              <v-list-item class="text-caption text-error" @click="signOut">
                Sign Out
              </v-list-item>
            </v-list>
          </v-menu>
          ...
          <v-tabs
            ...
            height="28"
          >
            <v-tab v-for="(year, index) in years" :key="index" :value="year">
              {{ year }}
            </v-tab>
          </v-tabs>
          ...
          <!-- create new -->
          <v-menu v-model="plusMenu">
            <template #activator="{ props }">
              <v-btn v-bind="props" size="36" color="primary" icon="mdi-plus" />
            </template>
            <v-list density="compact">
              <v-list-item
                class="text-caption"
      ...
      <template v-if="themes?.length">
        <v-col v-for="(item, index) in themes" :key="index" cols="12">
          <ThemeCard
            ...
            @edit-day="
              (day) => {
                Object.assign(editingDay, {
                  ...day,
                  theme: item.attributes.name,
                });
      ...
      <v-col v-else-if="!loading">
        <v-card flat class="pa-2 text-caption d-flex align-center">
          🎨 从
          <v-btn
            size="small"
            variant="plain"
  ...
  <!-- footer -->
  <v-container fluid class="d-flex py-0 justify-center">
    <v-col
      cols="12"
      md="7"
      lg="5"
  ...
  <record-day-dialog
    ...
    @reload="reloadTheme"
  />
  <app-about-dialog :key="aboutDialog.key" v-model="aboutDialog.model" />
</template>
<script setup>
...
const years = new Array(new Date().getFullYear() - 2022 + 1)
  .fill()
  .map((_, index) => index + 2022); // Boring Days was created in 2022
