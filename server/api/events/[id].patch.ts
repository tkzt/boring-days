...
export default defineEventHandler(async event => {
  ...
  const { rows } = await getDatabase().query(
    ...
    WHERE id = $7 AND user_id = $8 RETURNING id, title, description, event_date::text AS "eventDate", end_date::text AS "endDate", start_time AS "startTime", status`,
    [payload.title, payload.description, payload.eventDate, payload.endDate, payload.startTime, payload.status, id, user.id]
  )
  if (!rows[0]) {
